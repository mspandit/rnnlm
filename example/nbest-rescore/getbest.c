...
void main(int argc, char **argv)
{
    ...
    float lambda=0.75;	//for interpolation between rnnlm and normal lm scores
    LM_SCALE=14;	//LM scale
    WI_PENALTY=0;	//word insertion penalty
	...
	//score=acscore + lmscore*LM_SCALE + wcn*WI_PENALTY/(2.718);
	score=acscore + (lambda*rnnscore + (1-lambda)*lmscore)*LM_SCALE + wcn*WI_PENALTY/(2.718);
